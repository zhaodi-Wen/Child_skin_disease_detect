...
def evaluation_detail(sess,score,classIds,val_images_batch,val_labels_batch,val_nums):
    ...
    for _ in range(val_max_steps):
        val_x, val_y = sess.run([val_images_batch, val_labels_batch])
        val_score,val_classIds = sess.run([score,classIds], feed_dict={input_images: val_x, input_labels: val_y})
    ...
    with tf.Session() as sess:
        ...
        for i in range(max_steps + 1):
            ...
            if (i % snapshot == 0 and i > 0) or i == max_steps:
                ...
                #with tf.gfile.FastGFile(snapshot_prefix  +'-' +str(i)+'model'+'.pb', mode='wb') as f:
                #    f.write(constant_graph.SerializeToString())
                tf.train.write_graph(sess.graph_def,'./model4/checkpoints/','model'+'.pb',as_text=False)
            # 保存val准确率最高的模型
            if mean_acc > max_acc and mean_acc > 0.7:
                ...
                with tf.gfile.FastGFile(path +'-' +str(i)+'model'+'.pb', mode='wb') as f:
                    f.write(constant_graph.SerializeToString())
...
def train(train_record_file,
    ...
    with slim.arg_scope(mobilenet_v1.mobilenet_v1_arg_scope()):
        out, end_points = mobilenet_v1.mobilenet_v1(input_images, num_classes=labels_nums,dropout_keep_prob=keep_prob, is_training=is_training)
    ...
    classIds = tf.argmax(out, 1,output_type='int32',name='output')
...
if __name__ == '__main__':
    ...
    train_record_file = '/home/ubuntu/桌面/大创/Demo/train.tfrecords'
    val_record_file = '/home/ubuntu/桌面/大创/Demo/test.tfrecords'
    ...
    snapshot_prefix='./model4/checkpoints/model.ckpt'
